...
fn fixing_a_failed_include_triggers_recovery_without_touching_its_parent() {
    let sandbox = TempDir::new().expect("create config directory");
    let root = sandbox.child("config.toml");
    let child = sandbox.child("local.toml");
    root.write_str("[window]\ntitle = \"last good\"\n")
        .expect("write initial config");
    let current = load_config_from_path(root.path()).expect("load initial config");
    let mut reload = ConfigHotReload::new(root.path());
    let first_check = Instant::now() + CONFIG_HOT_RELOAD_INTERVAL;

    child
        .write_str("[window\ntitle = \"broken\"\n")
        .expect("write invalid include");
    root.write_str("include = [\"local.toml\"]\n\n[window]\ntitle = \"parent\"\n")
        .expect("add include");
...
#[test]
fn creating_a_missing_optional_include_triggers_reload() {
    let sandbox = TempDir::new().expect("create config directory");
    let root = sandbox.child("config.toml");
    let child = sandbox.child("optional.toml");
    root.write_str("include = [\"?optional.toml\"]\n\n[window]\ntitle = \"parent\"\n")
        .expect("write config with include");
    let mut reload = ConfigHotReload::new(root.path());
    ...
    let config = reload.reload_config().expect("load include");
    assert_eq!(config.window.title, "child");
}
...
fn creating_a_missing_required_include_recovers_the_failed_candidate() {
    let sandbox = TempDir::new().expect("create config directory");
    let root = sandbox.child("config.toml");
    let child = sandbox.child("required.toml");
    root.write_str("[window]\ntitle = \"last good\"\n")
        .expect("write initial config");
    let mut reload = ConfigHotReload::new(root.path());
    let first_check = Instant::now() + CONFIG_HOT_RELOAD_INTERVAL;

    root.write_str("include = [\"required.toml\"]\n\n[window]\ntitle = \"parent\"\n")
    ...
    assert_eq!(
        error.to_string(),
        format!("config file not found: {}", child.path().display())
    );
