...
fn terminal_engine_collects_osc52_clipboard_text() -> Result<()> {
    ...
    assert!(drain_clipboard_texts(&mut engine).is_empty());
    Ok(())
}
...
fn terminal_engine_applies_configured_default_colors_to_frame() -> Result<()> {
    ...
    assert_eq!(
        rendered_palette_color(&mut engine, 0, 'A')?,
        Some(RgbColor { r: 0, g: 1, b: 2 })
    );
    assert_eq!(
        rendered_palette_color(&mut engine, 1, 'B')?,
        Some(RgbColor { r: 3, g: 4, b: 5 })
    );
    Ok(())
}
...
proptest! {
    ...
    fn absolute_cursor_position_clamps_to_viewport(case in any::<CursorCase>()) {
        ...
        let expected_x = case.x.saturating_sub(1).min(case.cols - 1);
        let expected_y = case.y.saturating_sub(1).min(case.rows - 1);
        let expected_cursor_x = expected_x.saturating_add(1).min(case.cols - 1);
    ...
    fn relative_cursor_motion_clamps_to_viewport(case in any::<CursorCase>()) {
        let start_x = case.x.min(case.cols - 1);
        let start_y = case.y.min(case.rows - 1);
        ...
        engine.write_vt(
            format!("\x1b[{};{}H\x1b[{}{command}X", start_y + 1, start_x + 1, case.delta)
                .as_bytes(),
        );
        let frame = engine.extract_frame().expect("render frame");
        ...
        let expected_x = match case.direction {
            CursorDirection::Right => start_x.saturating_add(effective_delta).min(case.cols - 1),
        ...
        let expected_y = match case.direction {
            ...
            CursorDirection::Down => start_y.saturating_add(effective_delta).min(case.rows - 1),
            _ => start_y,
        };
        let expected_cursor_x = expected_x.saturating_add(1).min(case.cols - 1);
        prop_assert_eq!(occupied_cells(frame), vec![('X', expected_x, expected_y)]);
    ...
    fn terminal_default_tab_movement_lands_on_next_tabstop_or_right_edge(
        ...
        let expected_x = (((start_x / 8) + 1) * 8).min(case.cols - 1);
        let expected_cursor_x = expected_x.saturating_add(1).min(case.cols - 1);
...
#[test]
fn terminal_engine_decodes_text_and_character_sets() -> Result<()> {
    assert_render_cases(&[
...
fn terminal_engine_regenerates_palette_from_pristine_base_on_color_reload() -> Result<()> {
    let mut pristine = TerminalEngine::new(test_geometry(8, 1))?;
    let pristine_index_17 = rendered_palette_color(&mut pristine, 17, 'P')?;
    ...
    assert_ne!(
        rendered_palette_color(&mut engine, 17, 'G')?,
        pristine_index_17
    );
    ...
    assert_eq!(
        rendered_palette_color(&mut engine, 17, 'R')?,
        pristine_index_17
    );
    Ok(())
}
...
fn terminal_engine_generates_palette_from_color_config() -> Result<()> {
    ...
    let mut engine = terminal_engine_with_colors(test_geometry(2, 1), colors)?;
...
fn terminal_engine_supports_osc_color_operations() -> Result<()> {
    ...
    let reset_frame = engine.extract_frame()?;
    let reset_cells = collect_visible_cells(reset_frame, |text, cell| (text, cell.fg));
...
fn terminal_engine_supports_x11_color_names_in_color_operations() -> Result<()> {
    ...
    let invalid_frame = engine.extract_frame()?;
    let invalid_cells = collect_visible_cells(invalid_frame, |text, cell| (text, cell.fg));
...
#[test]
fn terminal_engine_supports_grapheme_width_and_wrap() -> Result<()> {
