...
#[test]
fn binding_parser_preserves_terminal_actions_and_errors() {
    for (input, action) in [
        ...
        assert_eq!(
            parse_binding(input).expect("action parses").action,
            action,
            "{input}"
        );
    }
...
#[test]
fn binding_parser_preserves_parameterized_actions() {
    for (input, action) in [
        (
            "a=search:needle",
...
#[test]
fn binding_formatting_is_canonical() {
    ...
    let ghost = String::from_utf8(vec![0xf0, 0x9f, 0x91, 0xbb]).expect("valid UTF-8");
    assert_eq!(
