...
use bootty_extension::{
    ...
    ModuleSourceRequest, SurfaceDeclaration, SurfacePlacement, SurfaceSnapshot, event_queue,
    head_branch, module_identities, preview_builtin_surfaces,
};
use pretty_assertions::assert_eq;
use static_assertions::const_assert_eq;

const_assert_eq!(ModuleColor::rgb(1, 2, 3).a, 255);
...
fn git_repo() -> (assert_fs::TempDir, std::path::PathBuf) {
...
#[test]
fn generation_publish_is_atomic_and_replacement_retires_the_old_generation() {
...
fn every_builtin_sidebar_module_previews_with_items() {
    for name in ["sessions", "codexbar"] {
        ...
        if name == "sessions" {
            assert!(
                surface.items.iter().any(|item| item.session_id.is_some()),
...
fn installing_an_integration_merges_into_a_file_it_never_owns() {
    let config = assert_fs::TempDir::new().expect("temporary config directory");
...
fn a_placed_file_lands_where_the_tool_reads_it_and_is_taken_back_on_uninstall() {
    let config = assert_fs::TempDir::new().expect("temporary config directory");
    let home = assert_fs::TempDir::new().expect("temporary home directory");
