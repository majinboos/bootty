...
fn legacy_module_stays_in_place_until_explicit_validated_import() {
    let config = assert_fs::TempDir::new().expect("config root");
...
#[rstest]
#[case("../status.luau")]
#[case("/tmp/status.luau")]
#[case("status.txt")]
fn module_identity_rejects_escape_absolute_and_wrong_extension(#[case] source: &str) {
    assert!(ModuleIdentity::parse(source).is_err(), "source: {source:?}");
}
...
#[test]
fn display_paths_abbreviate_home_without_abbreviating_lookalikes() {
...
fn builtin_and_local_modules_share_one_recursive_catalog() {
    let root = assert_fs::TempDir::new().expect("extension root");
...
proptest! {
    /// A relative Luau path made only of non-empty safe segments preserves the complete source
    /// path; its namespace is the path without the extension, with separators changed to dots.
    #[test]
    fn safe_nested_identity_preserves_path_and_derives_namespace(
        segments in prop::collection::vec("[a-z][a-z0-9_]{0,11}", 1..5),
    ) {
        let stem = segments.join("/");
        let source = format!("{stem}.luau");
        let identity = ModuleIdentity::parse(&source).expect("generated safe identity");

        prop_assert_eq!(identity.as_str(), source);
        prop_assert_eq!(identity.namespace(), stem.replace('/', "."));
    }
}
