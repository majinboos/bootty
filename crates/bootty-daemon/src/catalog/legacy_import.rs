...
mod tests {
    ...
    fn seed_wal_source(path: &Path) -> Result<()> {
        ...
        connection.execute_batch(
            "CREATE TABLE workspace_spaces (
             ...
             VALUES (1, 'snapshot-id', 'Snapshot', 0);
             INSERT INTO workspace_bindings (id, space_id, backend, remote)
             VALUES (1, 1, 'inherit', '{\"source\":\"local\"}');
    ...
    #[test]
    fn read_only_wal_import_uses_one_snapshot_and_retries_at_a_new_revision() -> Result<()> {
        let directory = tempfile::tempdir()?;
        let database_path = directory.path().join("session-order.sqlite3");
        let config_path = directory.path().join("config.toml");
        std::fs::write(&config_path, "[multiplexer]\nbackend = \"tmux\"\n")?;
        seed_wal_source(&database_path)?;

        let config = load_config_from_path(&config_path)?;
        let mut reader = open_legacy_connection(&database_path)?;
        ...
        let writer_transaction = writer.transaction()?;
        writer_transaction.execute(
            "UPDATE workspace_bindings SET backend = 'zellij' WHERE id = 1",
            [],
        )?;
        writer_transaction.execute(
            "UPDATE workspace_sessions SET name = 'after' WHERE binding_id = 1",
            [],
        )?;
        ...
        let binding_ids = bindings
            ...
            .collect::<HashSet<_>>();
        let sessions = load_sessions(&transaction, &binding_ids)?;
        let first_plan = build_plan(spaces, &bindings, &sessions, &config)?;
        transaction.commit()?;

        let write_error = reader
            .execute(
                "UPDATE workspace_bindings SET backend = 'rmux' WHERE id = 1",
                [],
            )
            .expect_err("legacy importer connection must be read-only");
        assert!(matches!(
            write_error,
            rusqlite::Error::SqliteFailure(error, _)
                if error.code == ErrorCode::ReadOnly
        ));
        assert_eq!(first_plan.spaces[0].backend, Backend::Tmux);
        assert_eq!(first_plan.spaces[0].sessions, ["before"]);

        let later_plan = load(&config_path, &database_path)?;
        assert_eq!(later_plan.spaces[0].backend, Backend::Zellij);
        assert_eq!(later_plan.spaces[0].sessions, ["after"]);
        Ok(())
    }
}
