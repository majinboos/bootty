...
use crate::control::{CliHerdrApi, HerdrApi};
...
fn run_policy_worker(config: PolicyWorkerConfig) {
    ...
    while let Ok(command) = rx.recv() {
        let result = match command {
            PolicyCommand::Focus(pane_id) => {
                policy_request(&session, "pane.focus", json!({"pane_id": pane_id}))
            }
        };
...
fn policy_request(
    ...
    CliHerdrApi::new(session).request(method, params)
}
...
impl HerdrPanePolicy {
    pub fn new(session: impl Into<String>) -> Self {
        ...
        Self {
            worker: PolicyWorker::new(session.clone()),
            session,
            focused_pane: None,
        }
    }
...
impl BackendPanePolicy for HerdrPanePolicy {
    ...
    fn start_terminal(
        ...
        start_attach_terminal(
            request,
            AttachLaunch {
                program: HERDR_PROGRAM.to_owned(),
                args,
                env_remove: Vec::new(),
                env: Vec::new(),
