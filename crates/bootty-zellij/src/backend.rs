    ...
    pub fn execute(&mut self, command: MuxCommand) -> Result<()> {
        match command {
            ...
            MuxCommand::RenameSession { session_id, name } => {
                self.run_owned(&["action".into(), "switch-session".into(), session_id])?;
                self.run_owned(&["action".into(), "rename-session".into(), name])?;
            }
            MuxCommand::DitchSession { session_id } => {
                self.run_owned(&["kill-session".into(), session_id])?;
            }
...
#[cfg(feature = "app")]
pub fn zellij_capabilities(scope: MuxScope) -> BindingCapabilityDescriptor {
