...
impl BoottyApp {
    ...
    fn show_binding_views(
        ...
        let corner_radius_px = chrome.pane_corner_radius;
        let inactive_dim = chrome.unfocused_terminal_dim.clamp(0.0, 1.0);
        let rects = self.state.binding_view_layout(area, gap);

        if ui.input(|input| input.pointer.primary_pressed()) {
            let scope = ui
                .input(|input| input.pointer.interact_pos())
                .and_then(|pos| {
                    if self.state.chrome_handle_contains(pos) {
                        return None;
                    }
                    rects
                        .iter()
                        .find_map(|(scope, rect)| rect.contains(pos).then_some(*scope))
                });
            self.state.resolve_binding_pointer_press(scope);
        }
        let focused = self.state.focused_binding_scope();
        let focused_widget_key = self.state.binding_view_widget_key(focused);
        self.focus_pane_widget(&focused_widget_key);
        ...
        for (scope, rect) in rects {
            let widget_key = state.binding_view_widget_key(scope);
            let label = state
                .binding_label_for_scope(scope)
                .unwrap_or_else(|| "Binding".to_owned());
            let is_focused = scope == focused;
            if state.binding_has_terminal(scope) {
                let transition_key = state.binding_terminal_transition_key(scope);
                let result = if is_focused {
                    terminal_widget.set_transition_key(transition_key);
                    state.terminal_for_scope_mut(scope).map(|source| {
                        terminal_widget.show_at_rect(
                            ui,
                            rect,
                            ("binding-terminal", &widget_key),
                            source,
                        )
                    })
                } else {
                    ...
                    widget.set_terminal_cursor_icon(*terminal_cursor_icon);
                    widget.set_transition_key(transition_key);
                    state.terminal_for_scope_mut(scope).map(|source| {
                        widget.show_at_rect(ui, rect, ("binding-terminal", &widget_key), source)
                    })
                };
                match result {
                    Some(Ok(surface)) => state.record_binding_surface(scope, surface),
                    Some(Err(error)) => state.record_render_error(error),
                    None => {}
                }
                let view = if is_focused {
                    terminal_widget.view_transform()
                } else {
                    pane_widgets
                        .get(&widget_key)
                        .map(TerminalWidget::view_transform)
                        .unwrap_or_default()
                };
                state.record_binding_view_transform(scope, view);
            } else {
                ui.painter().text(
                    rect.center(),
            ...
            paint_pane_corner_masks(ui.painter(), rect, corner_radius_px, background);
            let label_rect = Rect::from_min_size(rect.min, Vec2::new(rect.width(), 24.0));
            ui.painter()
                .rect_filled(label_rect, corner, egui::Color32::from_black_alpha(180));
            ui.painter().text(
        ...
        if !state.terminal_focused() {
            let dim = state.config().chrome.unfocused_terminal_dim.clamp(0.0, 1.0);
            ui.painter().rect_filled(
                area,
                0.0,
                egui::Color32::from_black_alpha((dim * 255.0) as u8),
            );
        }
    }
    ...
    fn show_fixed_layout(&mut self, ui: &mut egui::Ui) {
        ...
        if self.state.binding_count() > 1 {
            self.show_binding_views(ui, terminal_rect, palette, pane_backing_color);
            return;
        }
        if has_terminal {
