...
impl BindingRuntime {
    pub(crate) fn uses_native_terminal_layout(&self) -> bool {
        self.backend_policy.panes.topology != PaneTopology::Attach
    }

    pub(crate) fn pane_widget_key(&self, pane_id: &str) -> String {
    ...
    pub(crate) fn sync_terminal_panes(&mut self) -> Result<()> {
    ...
    pub(crate) fn native_multi_pane(&self) -> bool {
        self.current_pane_layout()
            .is_some_and(|layout| !layout.is_single())
    }

    pub(crate) fn focused_pane(&self) -> Option<String> {
        self.current_pane_layout()
            .map(|layout| layout.focused().to_owned())
    }

    pub(crate) fn pane_rects(&self, area: Rect, gap: f32) -> Vec<(String, Rect)> {
        self.current_pane_layout()
            .map(|layout| layout.rects(area, gap))
            .unwrap_or_default()
    }

    pub(crate) fn pane_dividers(&self, area: Rect, gap: f32) -> Vec<Divider> {
        self.current_pane_layout()
            .map(|layout| layout.dividers(area, gap))
            .unwrap_or_default()
    }

    pub(crate) fn focus_pane(&mut self, pane_id: &str) {
        let key = self.current_window_id();
    ...
    pub(crate) fn set_pane_ratio(&mut self, path: &[u8], ratio: f32, min_fraction: f32) {
        let key = self.current_window_id();
    ...
    pub(crate) fn terminal_runtime_for_pane(
        &mut self,
        pane_id: &str,
    ...
    pub(crate) fn pane_terminal_window_size<F>(&self, leaf_size: F) -> Option<(u16, u16)>
    where
    ...
    pub(crate) fn resize_native_layout_window(&mut self, cols: u16, rows: u16) -> Result<()> {
    ...
    pub(crate) fn focus_pane_relative(&mut self, delta: isize) {
        let key = self.current_window_id();
    ...
    pub(crate) fn close_focused_pane(&mut self, repaint: &RepaintHandle, pane_id: &str) {
