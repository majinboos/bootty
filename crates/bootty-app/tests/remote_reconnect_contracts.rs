...
fn manual_reconnect_targets_only_a_remote_binding() {
    ...
    let mut local_state = AppState::new(local, support::backends(), Arc::new(|| {}), None, None)
        .expect("start local state");
...
"#,
    ...
    let mut remote_state = AppState::new(remote, support::backends(), Arc::new(|| {}), None, None)
        .expect("start remote state");
