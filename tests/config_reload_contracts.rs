...
use bootty_config::{
    config::{ConfigState, load_config_from_path},
...
fn fixing_a_failed_include_triggers_recovery_without_touching_its_parent() {
    ...
    let mut config_state =
        ConfigState::new(load_config_from_path(&root).expect("load initial config"));
    ...
    config_state.reject(error.to_string());
    assert_eq!(config_state.current().window.title, "last good");
    assert!(
        config_state
            .last_error()
            .is_some_and(|error| error.contains("local.toml"))
    );
    ...
    config_state.accept(recovered);
    assert_eq!(config_state.current().window.title, "recovered from child");
    assert_eq!(config_state.last_error(), None);
}
